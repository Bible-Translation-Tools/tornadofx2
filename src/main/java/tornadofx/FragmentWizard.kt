...
abstract class FragmentWizard @JvmOverloads constructor(title: String? = null, heading: String? = null) : Fragment(title) {
    ...
    override val root = borderpane {
        addClass(FragmentWizardStyles.wizard)
        top {
            hbox {
                addClass(FragmentWizardStyles.header)
                removeWhen(showHeaderProperty.not())
                vbox(5) {
                    label(titleProperty)
                    label(headingProperty) {
                        addClass(FragmentWizardStyles.heading)
                        visibleWhen(titleProperty.isEqualTo(headingProperty).not())
                    }
                }
                spacer()
                label {
                    addClass(FragmentWizardStyles.graphic)
                    graphicProperty().bind(this@FragmentWizard.graphicProperty)
                }
            }
        }
        center {
            stackpane {
                addClass(FragmentWizardStyles.content)
        ...
        left {
            vbox {
                addClass(FragmentWizardStyles.stepInfo)
                removeWhen { showStepsProperty.not() }
                label(stepsTextProperty) {
                    addClass(FragmentWizardStyles.stepsHeading)
                ...
                vbox(5) {
                    bindChildren(pages) { page ->
                        ...
                        hyperlink("") {
                            ...
                            toggleClass(FragmentWizardStyles.bold, isPageActive)
                            action {
        ...
        bottom {
            buttonbar {
                addClass(FragmentWizardStyles.buttons)
    ...
    init {
        importStylesheet<FragmentWizardStyles>()
...
class FragmentWizardStyles : Stylesheet() {
    companion object {
